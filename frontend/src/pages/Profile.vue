    ...
    <div class="p-6 max-w-3xl mx-auto space-y-6">
      ...
      <div v-else class="space-y-6">
        ...
        <div class="glass-card p-6 flex items-center gap-5">
          <div
            class="w-20 h-20 bg-gradient-to-br from-primary-300 to-emerald-400 rounded-2xl flex items-center justify-center text-3xl font-bold text-white shadow-lg shrink-0">
            {{ initials }}
          </div>
          <div>
        ...
        <div class="glass-card p-6">
          <h3 class="font-semibold text-gray-700 dark:text-gray-200 mb-4">📝 Cập nhật thông tin</h3>
          <form @submit.prevent="saveProfile" class="space-y-4">
            ...
            <button type="submit" :disabled="savingProfile" class="btn-primary flex items-center gap-2">
              <span v-if="savingProfile"
                class="w-4 h-4 border-2 border-gray-400 border-t-gray-800 rounded-full animate-spin" />
              💾 Lưu thay đổi
            </button>
          </form>
        </div>
        ...
        <!-- Change password -->
        <div class="glass-card p-6">
          <h3 class="font-semibold text-gray-700 dark:text-gray-200 mb-4">🛡️ Đổi mật khẩu</h3>
          <form @submit.prevent="savePassword" class="space-y-4">
            ...
            <button type="submit" :disabled="savingPwd" class="btn-outline flex items-center gap-2">
              <span v-if="savingPwd"
                class="w-4 h-4 rounded-full border-2 border-primary-400 border-t-transparent animate-spin" />
              🛡️ Đổi mật khẩu
            </button>
          </form>
        </div>

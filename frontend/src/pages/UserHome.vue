<template>
  <div class="p-6 max-w-6xl mx-auto space-y-6">
    ...
    <!-- Welcome Banner -->
    <div class="rounded-3xl bg-gradient-to-br from-blue-700 via-blue-600 to-indigo-600 p-8 flex items-center justify-between shadow-2xl border border-blue-400/30">
      <div>
        <p class="text-blue-200 text-xs uppercase tracking-wide mb-2 font-semibold">Xin chào 👋</p>
        <h1 class="text-4xl font-bold text-white mb-1">{{ auth.user?.ten || 'Người dùng' }}</h1>
        <p class="text-blue-100 text-sm font-medium mt-2">
          {{ auth.user?.email }}
          <span v-if="auth.user?.so_dien_thoai"> · {{ auth.user?.so_dien_thoai }}</span>
        </p>
      </div>
      <button @click="showReport = true"
              class="hidden sm:flex items-center gap-2 bg-white/15 hover:bg-white/25 text-white font-semibold px-6 py-3 rounded-xl transition-all duration-300 backdrop-blur-sm border border-white/40 shadow-lg hover:shadow-xl hover:scale-105">
    ...
    <div class="glass-card p-7 shadow-md border border-gray-200/50 dark:border-gray-700/50">
      <h2 class="text-xl font-bold text-gray-800 dark:text-gray-100 mb-6">📊 Trạng thái báo cáo của bạn</h2>
    ...
    <div class="grid lg:grid-cols-3 gap-6">
      ...
      <div class="glass-card p-7 space-y-4 shadow-md border border-gray-200/50 dark:border-gray-700/50">
        <h2 class="text-lg font-bold text-gray-800 dark:text-gray-100 mb-5">⚡ Truy cập nhanh</h2>
        <button @click="showReport = true"
